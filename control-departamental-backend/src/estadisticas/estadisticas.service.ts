...
export class EstadisticasService {
  ...
  async findAllUser(id_user: UUID) {
    ...
    try{
      ...
      const pagosPendientes = pagos.filter(p => p.pago && p.pago.estado === ESTADO_PAGO.PENDIENTE).length;
      const pagosPagados = pagos.filter(p => p.pago && p.pago.estado === ESTADO_PAGO.PAGADO).length;
